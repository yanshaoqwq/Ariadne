...
fn known_section_aliases(section_id: &str) -> &'static [&'static str] {
    match section_id {
        ...
        "chapter_text" => &["当前章节正文"],
        "target_text" => &["待评价文本"],
        // U149：`评审意见` 是新名，`意见者输出` 是**必须保留的兼容别名**。
        // 「意见者」是我们发明的岗位名，现实创作行业里不存在——模型推不出
        // 「意见者输出」是什么，而「评审意见」它一读就懂。
        //
        // ⚠️ 上一轮（cb95e8f）把 `node_template.prudent.default` 里的占位符改成了
        // `{{评审意见}}`，却漏了这张别名表，于是产品自带的 prudent 默认模板**必然**
        // 渲染失败（`render_prompt_template` 对未知变量 fail-loud）——即
        // 「拖一个审稿裁断节点到画布上，运行必报变量无法解析」。
        // 教训：占位符改名不是文案改动，它是**渲染器契约的一半**，两边必须同时改。
        "critic_outputs" => &["评审意见", "意见者输出"],
        // 同理：「审慎者返修上下文」这个自造岗位名保留为别名，新名是「本次修改要求」。
        // 存量工作流的节点 config 存的是渲染前的模板字符串，只认新名会让
        // 已保存的工作流在下次运行时 fail-loud。
        "revision_context" => &["本次修改要求", "返修上下文", "审慎者返修上下文"],
        "revision_basis" => &["返修依据", "本次要改的地方"],
        _ => &[],
    }
}
