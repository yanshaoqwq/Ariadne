...
#[test]
fn node_registry_includes_builtin_start_node() {
    let registry = NodeRegistry::default();
    let start = registry.get("start").unwrap();

    assert_eq!(start.display_name.as_deref(), Some("ui.node.start"));
    assert!(start.execution_input_ports.is_empty());
    assert_eq!(start.execution_output_ports[0].name, "exec_out");
}
