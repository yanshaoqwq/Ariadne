...
本文件中当前仍有效、尚未实现的问题已经全部迁移到主审查报告 `项目检验报告/发布前全量代码审查报告-进行中-2026-07-11.md`：
...
A5 的 `_suppressAutoModeSave` 旧实现已不存在，判为已解决，不迁移。自此以后本文件只作为历史证据，活动问题以主审查报告为唯一账本；待主报告对应项全部验证关闭后，本文件可删除。
