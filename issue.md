...
> ## ⚠️ 2026-08-17 复核：本文件已可归档，不要再按它施工
>
> 全部有效未解决项早在 2026-07-11 就迁移到
> [发布前全量代码审查](项目检验报告/发布前全量代码审查/README.md)（见下方迁移表）。
> 此后所有工作都在那套编号（U1xx）下进行，**本文件未同步过**，
> 因此它记的状态一律是 2026-07-11 的快照。
>
> 本次复核只核实了当时列为「尚不能删除」的那批，逐项对照当前代码：
>
> | 旧编号 | 当时状态 | 现在 |
> |---|---|---|
> | A12 分块 TextBox 破坏跨块选择 | 新发现·确认存在 | ✅ 已解决：正文已换 `SelectableTextBlock`，跨块选区由 `ReadingSelectionAggregator` 合并（U132/U133），「分块：N」文案已移除 |
> | A1 EN/JA 零翻译 stub | 仍存在 | ✅ 已解决：`display_name.en/ja.json` 已补齐，含 44 个 `tool.*` 键 |
> | A10 日志页只筛级别/关键词 | 仍存在 | ✅ 已解决：U137 加 id chip 筛选，U158 让成功运行也可见 |
>
> ⇒ **本文件不再承载任何独有信息**。保留它只是为了让旧链接不断；
> 要查当前状态一律去 `项目检验报告/发布前全量代码审查/13`（问题与证据）
> 与 `13A`（施工说明与已修记录）。
