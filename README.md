...
Ariadne 是一款面向长篇小说创作的桌面 AI 工作台。它把作品正文、项目记忆、工作流编排、模型配置、模板集市和版本存档放在同一个应用里，适合用来规划、撰写、修订和管理长篇项目。
...
这里用于阅读、修改和导入章节正文。右侧面板包含目录、大纲入口、项目 AI 和项目长期记忆。
...
- 模型：模型供应商、可用模型、默认模型、向量检索模型和 API Key。
- 预设：不同节点类型的新建默认模型、预算和超时，以及模板仓库入口。
- 自动化：预算、Auto Mode、确认项策略和运行限制。
- 权限控制：网络、工具、路径读写等权限。
- 个性化：主题、颜色、工作台显示偏好。
- 杂项：检索参数、Git 跟踪、语言、诊断状态。
...
这里用于查看存档点和 checkpoint。建议在重要章节完成、结构大改、批量润色前创建手动存档。

## 使用建议

- 模型配置完成前，不要直接运行需要 LLM 的工作流。
...
应用支持中文、英文和日文界面。可以在“配置 > 杂项 > 语言”中切换。
